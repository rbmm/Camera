...
BOOL CEncTcp::OnRecv(PSTR Buffer, ULONG cbTransferred)
{
	...
	ULONG cbExtra = 0, cbReaded = (ULONG)(Buffer - buf);

	if (cbReaded < sizeof(PACKET_HEADER))
	{
		cbTransferred += cbReaded, Buffer = buf;
__loop:
		if (cbTransferred < sizeof(PACKET_HEADER))
		{
			packet->setDataSize(cbTransferred);
			DbgPrint("%hs(%u): !! %x %x %x\r\n", __FILE__, __LINE__, cbTransferred, ph->size, packet->getFreeSize());
			return TRUE;
		}

		packet->setDataSize(0);

		if (ph->size > packet->getFreeSize())
		{
			DbgPrint("%hs(%u): !! %x %x %x\r\n", __FILE__, __LINE__, cbTransferred, ph->size, packet->getFreeSize());
			return FALSE;
		}
